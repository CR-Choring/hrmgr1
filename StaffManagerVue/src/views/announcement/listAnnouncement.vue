<template>
  <div class="app-container">
    ...
    <el-form :inline="true" class="demo-form-inline">
      ...
      <el-button type="primary" style="font-size: 16px;" @click="getAnnouncementListPage()"> 查询</el-button>
      <el-button type="default" @click="resetData()" style="font-size: 16px;"> 清空</el-button>
    </el-form>
    <el-table :data="list" element-loading-text="数据加载中" fit highlight-current-row>
      ...
      <el-table-column label="操作" width="360" align="center" fixed="right">
        <template slot-scope="scope">
          <router-link :to="'/announcement/updateAnnouncement/' + scope.row.id" v-if="roles.includes('人事经理') || roles.includes('管理员')">
            <el-button type="primary" style="font-size: 16px;">修改</el-button>
          </router-link>
          <router-link :to="'/inform/showAnnouncement/' + scope.row.id">
            <el-button type="info" style="font-size: 16px;">查看</el-button>
          </router-link>
          <el-button type="danger" style="font-size: 16px;" @click="deleteAnnouncementById(scope.row.id)" v-if="roles.includes('人事经理') || roles.includes('管理员')">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    <!-- 分页 -->
    <el-pagination :current-page="page" :page-size="limit" :total="total" style="padding: 30px 0; text-align: center"
      layout="total, prev, pager, next, jumper" @current-change="getAnnouncementListPage" />
  </div>
</template>
